...
LC_raster <- raster(here::here("G:/My Drive/taf/cours fac/Environemental data mining/switzerland/switzerland/LandCover_Type_Yearly_500m_v6/LC1/MCD12Q1_LC1_2019_001.tif"))
...
points <- data.frame(matrix_full$longitude,matrix_full$latitude)
LC <- extract(PFT_raster, points, method='simple')

matrix_LC<- as.data.frame(LC_raster, xy = TRUE, na.rm = TRUE) 

matrix_full$LC <- round(LC)

########  https://lpdaac.usgs.gov/documents/101/MCD12_User_Guide_V6.pdf  (corresponding value)
